...
from leanimum.models.utils.actions_toolcall_response import BASH_TOOL_RESPONSE_API
...
def test_response_api_model_cost_tracking():
    ...
    with (
        patch("leanimum.models.portkey_response_model.Portkey", mock_portkey_class),
        patch.dict(os.environ, {"PORTKEY_API_KEY": "test-key"}),
        patch("leanimum.models.portkey_response_model.litellm.cost_calculator.completion_cost", return_value=0.05),
    ):
        mock_response = Mock()
...
def test_response_api_model_zero_cost_assertion():
    ...
    with (
        patch("leanimum.models.portkey_response_model.Portkey", mock_portkey_class),
        patch.dict(os.environ, {"PORTKEY_API_KEY": "test-key"}),
        patch("leanimum.models.portkey_response_model.litellm.cost_calculator.completion_cost", return_value=0.0),
    ):
        mock_response = Mock()
