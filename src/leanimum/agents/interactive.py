...
from leanimum.agents.default import AgentConfig, DefaultAgent
from leanimum.agents.utils.prompt_user import _multiline_prompt, prompt_session
from leanimum.exceptions import LimitsExceeded, Submitted, TimeExceeded, UserInterruption
...
class InteractiveAgent(DefaultAgent):
    ...
    def add_messages(self, *messages: dict) -> list[dict]:
        ...
        for msg in messages:
            ...
            if role == "assistant":
                console.print(
                    f"\n[red][bold]Leanimum-agent[/bold] (step [bold]{self.n_calls}[/bold], [bold]${self.cost:.2f}[/bold]):[/red]\n",
                    end="",
                    highlight=False,
                )
            else:
